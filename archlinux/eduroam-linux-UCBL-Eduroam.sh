...
function setup_environment {
  bf=""
  n=""
  ORGANISATION="Université Claude Bernard Lyon1"
  URL="votre page de support local eduroam"
  SUPPORT="wifi@univ-lyon1.fr"
if [ ! -z "$DISPLAY" ] ; then
...
if ! ask "Cet installateur ne fonctionnera correctement que si vous êtes membre de ${bf}Université Claude Bernard Lyon1${n} et du groupe d'utilisateur : ${bf}Eduroam.${n}" "Continuer" 1 ; then exit; fi
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius.univ-lyon1.fr'})
        server_name = 'radius.univ-lyon1.fr'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
        ...
        s_8021x = dbus.Dictionary({
            ...
            'anonymous-identity': 'anonymous@univ-lyon1.fr',
        })
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="radius.univ-lyon1.fr"
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
  anonymous_identity="anonymous@univ-lyon1.fr"
}
EOFW
